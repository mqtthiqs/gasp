...
\begin{frame}{A paradoxical situation}  
  ...
  \pause
  \begin{block}{Mission}
    \large \it Isn't it time to adapt these tools to proofs?
  \end{block}
  {\Large }
\end{frame}
\hrule
...
to reason about formal languages and especially proof
languages: some of these tools are even dedicated to the formalization
of proof systems and their properties. But we still use the legacy
generic tools to manage our developments.
...
\begin{frame}{The impact of changes}
  \vspace{0.5em}
  \begin{center}
    \only<1-3>{\includegraphics[width=0.75\textwidth]{images/coqide.png}}%
...
of the constructions. It can be viewed as a kernel for a typed version
control system.
...
\begin{frame}{Inspiration: the \textsf{git} storage model}
  ...
  Let's design a \emph{typed} \textsf{git} to do incremental type-checking
\end{frame}
