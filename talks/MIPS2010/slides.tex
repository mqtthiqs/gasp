...
\begin{frame}{Incremental type-checking, incrementally}%
  ...
  \begin{block}{Typing}
      ...
      \vspace{0em}
      \begin{center}
        \begin{overlayarea}{0.8\textwidth}{5em}
          \only<-6>{ $ \Gamma \vdash t : u $ \hfill `` In environment
            $\Gamma$, term $t$ has type $u$ '' } \only<7>{ $ \Gamma
            \vdash t : u \alert{\Rightarrow \Delta} $ \hfill ``
            From \alert{repository} $\Gamma$, term $t$ of type $u$ \alert{leads to
            the new repository $\Delta$}'' }
        \end{overlayarea}
      \end{center}
  \end{block}
\end{frame}
...
\begin{frame}{Typing rules}
  ...
  \begin{mathpar}
    \large
    \infer                      % prod
    {\Gamma\vdash t : *\uncover<2->{ \Rightarrow \_} \and \Gamma[x:t]\vdash u :
      *\uncover<2->{\Rightarrow\Delta}}
    {\Gamma\vdash(x:t)\cdot u : *\uncover<2->{\textcolor{gray}{\Rightarrow\Delta}}}
    \and
    \uncover<3->{               %init
      \infer{ }
      {\Gamma\vdash x : t \uncover<4->{\Rightarrow\Gamma}}
...
\begin{frame}{Typing rules}
  \begin{block}{Equality binder}
    \begin{mathpar}
      \large
      \infer
      {\Gamma\vdash a : u \Rightarrow \_ \and \Gamma[x=a:u]\vdash t :
        *\Rightarrow\Delta }
      {\Gamma\vdash (x=a)\cdot t : * \Rightarrow\Delta}
      \quad {\small [y=a:u] \notin\Gamma}
      \and \pause
      \infer
      {\Gamma\vdash t \{y/x\} : * \Rightarrow\Delta}
      {\Gamma\vdash(x=a)\cdot t : * \Rightarrow\Delta}
  ...
  \infer
   ...
   \infer                      % prod
       {\Gamma\vdash t : *\uncover<2->{ \Rightarrow \_} \and \Gamma[k:t]\vdash u\{x/k\} :
         *\uncover<2->{\Rightarrow\Delta}}
       {\Gamma\vdash(x:t)\cdot u : * \textcolor{gray}{\Rightarrow\Delta}}
       \quad{\small k=\nu()}
