...
\begin{frame}{Incremental computation}
  ...
  \item[\itminus] {\footnotesize Mixes two goals:}
    \begin{itemize}
    \item<4-> {\large derivation synthesis} \footnotesize (the typing
      algorithm)
    \item<4-> {\large object reuse} \footnotesize (sharing common
      subderivation)
    \end{itemize}
...
\begin{frame}{Incremental computation}
  ...
  \begin{block}{Bidirectional incremental updates}
    ...
    \begin{tikzpicture}[node distance=4em, auto, >=latex]
      ...
      \alt<3>{ \draw[->] [out=0, in=0, color=red] (delta) edge
        (commit); }{ \draw[->] (M') edge (commit); }
