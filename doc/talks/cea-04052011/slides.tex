...
\begin{frame}{A data-oriented notion of delta}
  \begin{block}{The \alt<+>{first-order}{binder} case}
    ...
    A delta is a term $t$ with \emph{metavariables} \alert{$\mmeta,
      \mmmeta$}
    \alt<1>{, defined in the repository}{and \emph{boxes} \alert{$\ibox t {\mmeta.n} y u$} to
  ...
  \begin{onlyenv}<4>
    ...
    \begin{block}{Syntax}
      \begin{align*}
        ...
        \mf &\gequal { \prd\mv\mf\mf \gor \lapp\mf\ma \gor \mcf } \\
        \mo &\gequal { \lam\mv\mf\mo \gor \lapp\mo\ma \gor \mv \gor
          \mco } \\
...
\begin{frame}{The delta language}
    ...
    \begin{column}{0.6\textwidth}
      \begin{block}{Syntax} \vspace{-2em}
        \begin{align*}
          \mk &\gequal { \prd\mv\mf\mk \gor \type } \\
          \mf &\gequal { \prd\mv\mf\mf \gor \lapp\mf\ma \gor \mcf } \\
          \mo &\gequal { \lam\mv\mf\mo \gor \lapp\mo\ma \gor \mv \gor \mco
            \gor \alert{\mmeta \gor \ibox\mo{\mmeta.n}\mv\mo}
          } \\
        \ma &\gequal { \lnil \gor \lcons\mo\ma} \\[0.5em]
          \msi &\gequal { \enil \gor \ebinddecl\msi\mco\mf \gor
            \ebinddecl\msi\mcf\mk }
  ...
  \begin{block}{Informally}
    ...
    \item $\jindex\msi{\pair\mr\me}{\ibox\mo{\mmeta.n}\mv\mmo}\To\mmr$
      {\footnotesize means}\\ “Meta $\mmeta$ has the form
      $t\ldots(\ulam \mv\ldots)^n\ldots$ in $\mr$. Make all
...
\begin{frame}{Monadic LF}
  ...
  \begin{align*}
    ...
    \mo &\gequal \ret\mval \gor \ms\vdash\lapp\mval\ma \\
    \ma &\gequal \lnil \gor \lcons\mval\ma \\
    \mval &\gequal \mco \gor \mv \gor \lam\mv\mf\mo \\
    \ms\ &\ :\ \mv\mapsto{\lapp\mval\ma} \\
    \msi &\gequal { \enil \gor \ebinddecl\msi\mco\mf \gor
      \ebinddecl\msi\mcf\mk }
  \end{align*}
\end{frame}

\begin{frame}{\alt<1>{Type annotation}{The repository language}}
  \begin{block}{Remark}
  ...
  \begin{align*}
    \mk &\gequal \prd\mv\mf\mk \gor \type \\
    \mf &\gequal \prd\mv\mf\mf \gor \ms\vdash\lapp\mcf\ma \\
    \alt<1>\mo{\alert\mr} &\gequal \ret\mval \gor \ms\vdash\laapp\mval\ma{\alert{\lapp\mcf\ma}} \\
    \ma &\gequal \lnil \gor \lcons\mval\ma \\
    \mval &\gequal \mco \gor \mv \gor \lam\mv\mf{\alt<1>\mo{\alert\mr}} \\
    \ms\ &\ :\ \mv\mapsto{\laapp\mval\ma{\alert{\lapp\mcf\ma}}} \\
    \msi &\gequal { \enil \gor \ebinddecl\msi\mco\mf \gor
      \ebinddecl\msi\mcf\mk }
  \end{align*}
  \pause
...
\begin{frame}{Commit}
  \inXLF {\Large
  \[ \jindex{\msi^+}{\pair{\mr^+}{\enil^+}}{\typ{\mo^+}{\mf^-}} \To{\mr^-} \]}
\begin{block}{What does it do?}
  ...
  \item with the adapted rules for variable \& box:
    \begin{mathpar}
      \infer[Var]{ \elookdef\me\mv\mf \and \text{or} \and \elookdef\ms\mv\mf }{
