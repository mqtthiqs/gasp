...
\newcommand\jannot[3]{\jlang{}{#1}{{#2}\rightarrow{#3}}}
\newcommand\jannotK[3]{\jannot{#1}{#2}{#3}}
\newcommand\jannotA[3]{\jannot{#1}{#2}{#3}}
\newcommand\jannott[3]{\jannot{#1}{#2}{#3}}
...
\begin{figure}
  \begin{align*}
    ...
    \sigma &\gequal { \snil \gor \scons \sigma x t } %TODO
  \end{align*}
...
Let us move on to the translation.
% parler de \Sigma

\begin{figure}
  \underline{Judgements}
  \begin{gather}
    \jannotK\Gamma {\mathbf K} {K'} \\
    \jannotA\Gamma {\mathbf A} {A'} \\
    \jannott\Gamma {\mathbf t} {t'} \\
    \jannoth\Gamma {\mathbf h} {h'} A \\
    \jannotl\Gamma {\mathbf l'} A l {l''} A'
  \end{gather}
  \underline{Translation}
  \begin{mathpar}
    %% kind
    \infer{ }{\jannotK\Gamma\type\type}
    \and
    \infer{
      \jannotK\Gamma A {A'} \and \jannotK{\econs\Gamma{x:A'}} K {K'}
    }{
      \jannotK\Gamma{\prd x A K}{\prd x {A'}{K'}}
    }
    \and
    %% fam
    \infer{
      \jannotl\Gamma\lnil{\elookup\Sigma a} l {l'} K
    }{
      \jannotK\Gamma{\lapp a l}{\lapp a {l'}}{K}
    }
    \and
    \infer{
      \jannotA\Gamma A {A'} \and \jannotA{\econs\Gamma{x:A}}{B}{B'}
    }{
      \jannotA\Gamma{\prd x A B}{\prd x {A'}{B'}}
    }
    \and
    %% obj
    \infer{
      \jannotA\Gamma A {A'} \and \jannott{\econs\Gamma{x:A'}} t {t'}
    }{
      \jannott\Gamma{\lam x A t}{\lam x {A'}{t'}}
    }
    \and
    \infer{
      \jannoth\Gamma h {h'} A \and
      \jannotl\Gamma\lnil A l {l'} {A'}
    }{
      \jannott\Gamma{\lapp h l}{\lapp{h'}{l'}}
    }
    \and
    %% args
    \infer{  }{
      \jannotl\Gamma l A \lnil l A
    }
    \and
    \infer{
      \jannott\Gamma t {t'} \and
      \jannotl\Gamma{\lcons {x=t'} {l'}} B l {l''} C
    }{
      \jannotl\Gamma{l'}{\prd x A B} {\lcons t l} {l''} C
    }
    \and
    %% head
    \infer{ }{
      \jannoth\Gamma x x {\elookup\Gamma x}
    }
    \and
    \infer{ }{
      \jannoth\Gamma c c {\elookup\Sigma c}
    }
    \and
    \infer{ }{
      \jannoth\Gamma a a {\elookup\Sigma a}
    }
    \and
    \infer{
      \jannott\Gamma t {t'}
    }{
      \jannoth\Gamma t {t'} {(\typeof {t'})}
    }
  \end{mathpar}
  \underline{where}
  \begin{align*}
