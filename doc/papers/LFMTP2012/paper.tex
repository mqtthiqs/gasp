...
\begin{figure*}
  ...
  \begin{mathpar}
    ...
    \infer{
      ...
      \jm\mr\Gamma F {\app{\cst a} S} \mmr {M'}
    }
...
\begin{align*}
  ...
  U &\gequal
  F \gor
  \matchin U \Gamma \casenobar {\app {H} {\vec {\var x}}} U\ |\ \ldots\
  \case {\app {H} {\vec {\var x}}} U
\end{align*}

This language is un(i)typed, in that all \LF\ objects have the same
implicit type $\cst{obj}$: there is no static guarantee that terms
produced by a constant $\cst c : A_1\to A_2$ will have type $A_2$; it
is also non-terminating: we could declare a constant $\cst c : \cst
a\to \cst a = \lam x \app {\cst c} {\var x}$. We expose here only the
minimal building blocks, but other features could be added at will to
this language. You can think of defined constants as black boxes
taking and producing terms, and being able to call other black
boxes. We detail in section \ref{sec:implem} how to embed
\sysname{OCaml} as computation calculus.

Nevertheless, in order to maintain the well-typing of the generated
term, we want to maintain the invariant that arguments fed to these
defined constant as well as the computation result have the announced
type. We thus maintain \emph{dynamic typing} of computations.
