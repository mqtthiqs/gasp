...
elimination ($\recb M N x y P$), and subtyping those types. He (or
...
\cst{even}$, but $\var{add}$ awaits $\cst{nat}$ arguments. When the
system will generate this derivation, it will insert a coercion at
these two points.
% TODO j'en ss la ds la relecture F<
Taking advantage of $\md_2$ and $\md_4$ the system
succeeds with a new derivation made out of these, without having to
regenerate them because it knows that $\finfer{\fget\md} = \md$.
