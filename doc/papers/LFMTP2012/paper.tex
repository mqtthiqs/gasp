...
hard to get correct, but \sysname{T_{<:}} is a typed language; he
...
The idea of untrustedly generating certificates verified \emph{a
  posteriori} is well-known to \sysname{LCF}-style proof assistants:
...
\sysname{LCF} architecture: an untrusted proof generator is composed
...
with a \sysname{CamlP4} syntax extension, offering the \emph{commit}
and \emph{checkout} operations on repositories, given an \LF\
signature and \sysname{OCaml} implementations for the defined
functions.
