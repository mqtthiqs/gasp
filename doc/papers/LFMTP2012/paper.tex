...
variables, write $A\to B$ for $\prd x A B$ when $x\notin \FV{B}$,
$\meta X$ for $\smeta X \msubstnil$, and adopt indifferently list or
map notations for $\sigma$ and $\Gamma$, \ie\ write $\sigma(x) = M$ or
$\Gamma(x) = A$.
...
sliceable $\annsliceable$ (each application of this constant will have
its own name in the repository), non-sliceable $\annnonsliceable$ (we
don't create a name for applications of this constant), or defined
$\anndefined{F}$ (see \ref{sec:computational-calculus}).
\begin{align*}
  L &\gequal
  \annsliceable \gor
  \annnonsliceable \gor
...
\begin{figure*}
  ...
  \begin{mathpar}
    \infer{
      \jstr{\Gamma\vdash\tp M A}{\Gamma'\vdash\tp {M'} {A'}}\sigma
      \and
      \text{$\meta X$ fresh in $\Delta$}
    }{
      \jp{(\Delta; \smeta Z \sigma)}\Gamma M A {(\mapadd\Delta{\meta
          X}{(\Gamma'\vdash M':A')}; \smeta X \sigma)}
    }
  \end{mathpar}
...
objects. This language is untyped, but can contain. % TODO??
\begin{align*}
  T &\gequal
  F \gor
  \matchin T \Gamma \casenobar {\app {\cst c} {\vec x}} T\ |\ \ldots\
  \case {\app {\cst c} {\vec x}} T
\end{align*}
