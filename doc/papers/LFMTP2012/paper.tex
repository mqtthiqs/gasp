...
untrusted procedure $\finfer M$ to infer a typing derivations out of
programs $M$; it doesn't have to be complete, or even sound, and 2/ a
trusted and fast kernel $\fcheck{\md}$ to verify these derivations in
a given set of inference rules $\Sigma$. This is a \emph{certifying}
scheme: if $\fcheck{\finfer M} = \cst{true}$ then there really
\emph{is} a derivation $\md$ for $M$. Of course the converse is not
...
run it: writing a program longer than a few dozen of lines becomes a
...
\item first, they ignore the higher-order nature of program syntax,
  ...
  ground derivation of $\vdash 2+2 : \cst{nat}$ is still valid when
  put under a binder $\vdash\lam x 2 + 2 - x:\cst{nat}\to\cst{nat}$
...
\item finally, there is no way to \emph{a posteriori} validate the
  ...
  incremental one.
\end{itemize}
It just so happens that, if we consider as above a type-checker as a
derivation-generating procedure, and storing these derivations between
calls to the procedure, we could recognize already-built
subderivations for subterms common to a checked and a new version to
check of the program, and graft these pieces of derivation to form a
new, large one for the whole modified program, saving the regeneration
of all equal subterms' derivations in compatible contexts. This way we
maintain the verifiable witness of well-typing for the program, and
take advantage of the quotients made on higher-order syntax.
...
\item an untyped, unsafe calculus to write derivation-generating type
  checkers, based on a notion of function inverse %TODO le nom?
  to refer an already-computed value;
...
$\fget\md$ from the derivation to the program it types, what needs
...
already-seen input. Since       %TODO


To simplify the problem,
we separate the task in:
\begin{itemize}
\item from a modified program, generate the \emph{delta} against the
  previous version, that is, observing and linking to unchanged
  derivations.
\item given a \emph{delta}, construct a new derivation and verify it.
\end{itemize}
...
\item conciseness of the certificates, since they are usually stored
  or transferred remotely; in particular,
