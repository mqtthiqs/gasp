...
\paragraph{A backend for incremental type checking}
...
\paragraph{A language to write incremental type-checkers}
...
the abstract syntax, the typing rules and the typing algorithm in
order to build the incremental type checker. How should he write the
algorithm? Usually, it takes a program and an environment and returns
nothing but we know that we want to return a derivation, and take
advantage of a higher-order representation of the syntax, so it
shouldn't be necessary to thread any explicit environment. %TODO
$$
