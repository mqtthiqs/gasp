	...
	| SLF.Box (t,p,u) ->
	  begin match term env t with
		...
		| _ -> Errors.not_an_obj u in
	      LF.Obj(LF.OBox(t, Option.map (fun x,n -> mk_variable x, mk_variable n) p, s))
...
let rec from_obj' = function
  ...
  | LF.OBox (t,p,u) -> SLF.Box (from_obj t, Option.map (fun x,n -> of_variable x, of_variable n) p, from_obj u)
