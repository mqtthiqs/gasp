...
let rec conv_obj subst ren t u = 
  match t, u with
    | OMeta _, OMeta _ -> assert false
    | OConst x, OConst y when x=y -> ()
    | OVar x, _ -> 
	let x = Ren.find x ren in
	begin try conv_obj (Subst.remove x subst) ren (Subst.find x subst) u
	with Not_found -> 
	  match u with OVar y when x = y -> () | _ -> assert false
	end
    | _, OVar y ->
	begin try conv_obj (Subst.remove y subst) ren t (Subst.find y subst)
	with Not_found ->
	  match t with OVar x when x = y -> () | _ ->  assert false
	end
    | OApp(t,u), OApp(t',u') ->
	conv_obj subst ren t t';
	conv_obj subst ren u u'
    | _ -> assert false

let rec conv_fam subst ren a b =
  match a, b with
    | FConst c, FConst d when c = d -> ()
    | FProd(x,a,b), FProd(y,c,d) ->
	conv_fam subst ren a c;
	conv_fam subst (Ren.add x y ren) b d
    | FApp (a,t), FApp(b,u) -> conv_fam subst ren a b; conv_obj subst ren t u
    | _ -> Errors.not_convertible_fam (SLF_LF.from_fam a) (SLF_LF.from_fam b)

let rec conv_kind subst ren k l = 
  match k,l with
    | KType, KType -> ()
    | KProd(x,a,k), KProd(y,b,l) -> 
	conv_fam subst ren a b;
	conv_kind subst (Ren.add x y ren) k l
    | _ -> assert false

let conv_obj subst t u = 
  Format.printf "= CO %a ⊢ %a == %a@." 
    Subst.pp subst SLF_pp.term (SLF_LF.from_obj t) SLF_pp.term (SLF_LF.from_obj u);
  conv_obj subst Ren.empty t u
let conv_fam subst a b = 
  Format.printf "= CF %a ⊢ %a == %a@."
    Subst.pp subst SLF_pp.term (SLF_LF.from_fam a) SLF_pp.term (SLF_LF.from_fam b);
  conv_fam subst Ren.empty a b
let conv_kind subst k l = 
  Format.printf "= CK %a ⊢ %a == %a@."
    Subst.pp subst SLF_pp.term (SLF_LF.from_kind k) SLF_pp.term (SLF_LF.from_kind l);
  conv_kind subst Ren.empty k l
...
let rec obj' sign env subst t : fam * subst = 
  ...
  | OLam (x,a,t) -> 
      ...
      conv_kind subst k KType;
      let b, _ = obj sign ((Name.variable_for x,a)::env) subst t in
      FProd(x,a,b), subst
  | OApp (t,u) ->
      begin match obj sign env subst t with
	| FProd(x,a,b), subst ->
	    (let a', _ = obj sign env subst u in
	    conv_fam subst a a');
	    b, Subst.add_name x u subst
	| _ -> Errors.bad_application (SLF_LF.from_obj t)
      end
  | OMeta _ -> assert false

and obj sign env subst t =
  Format.printf "[ O %a ⊢ %a@." Subst.pp subst SLF_pp.term (SLF_LF.from_obj t);
  let a, subst' = obj' sign env subst t in
  Format.printf "] O %a : %a => %a@." SLF_pp.term (SLF_LF.from_obj t) SLF_pp.term (SLF_LF.from_fam a) Subst.pp subst;
  a, subst'
...
and fam' sign env subst = function
  ...
  | FApp (a,t) -> 
	...
	| KProd(x,a,k), subst -> 
	    (let a', subst = obj sign env subst t in
	     conv_fam subst a a');
	    k, Subst.add_name x t subst
	| KType, _ -> assert false
	    
and fam sign env subst a =
  Format.printf "[ F %a ⊢ %a@." Subst.pp subst SLF_pp.term (SLF_LF.from_fam a);
  let k, subst' = fam' sign env subst a in
  Format.printf "] F %a : %a => %a@." SLF_pp.term (SLF_LF.from_fam a) SLF_pp.term (SLF_LF.from_kind k) Subst.pp subst;
  k, subst'

let rec kind' sign env subst k =
  match k with
    | KType -> ()
    | KProd(x,a,k) -> 
	let k, subst = fam sign env subst a in
	conv_kind Subst.empty k KType;
	kind sign ((Name.variable_for x,a)::env) subst k

and kind sign env subst k =
  Format.printf "[ K %a ⊢ %a@." Subst.pp subst SLF_pp.term (SLF_LF.from_kind k);
  let () = kind' sign env subst k in
  Format.printf "] K %a : ok." SLF_pp.term (SLF_LF.from_kind k)
