...

{
  ...
  let ocaml_keywords =
    ...
    List.iter (fun s -> Hashtbl.add h s ())
      [
        ".PHONY"; ".PRECIOUS"; "ifeq"; "ifdef"; "ifndef"; "endif"
      ];       h
...
rule pp fmt = parse
  ...
  | '^' { fprintf fmt "\\ensuremath{\\hat{}}\hspace{0.3cm}"; pp fmt lexbuf }
  | '&'  { fprintf fmt "\\&{}"; pp fmt lexbuf }
  | ':'  { fprintf fmt "\\ensuremath{\\colon\\hspace{-0.05cm}}"; pp fmt lexbuf }
...
{
  let makefile_alltt fmt s =
    fprintf fmt "\\begin{alltt}";
    pp fmt (from_string s);
    fprintf fmt "\\end{alltt}%%\n"

  let makefile_sf fmt s =
    fprintf fmt "\\bgroup\\sf\\medskip\\begin{flushleft}\n";
    start_of_line fmt (from_string s);
    fprintf fmt "\\end{flushleft}\\medskip\\egroup\\noindent\n"

  let () = Pp.add_pp_environment "makefile-tt" makefile_alltt
  let () = Pp.add_pp_environment "makefile-sf" makefile_sf
  let () = Pp.add_pp_environment "makefile" makefile_sf
  ...
  let () = Pp.add_pp_macro "makefile-tt" texttt
  let () = Pp.add_pp_macro "makefile-sf" textsf
  let () = Pp.add_pp_macro "makefile" textsf
}
