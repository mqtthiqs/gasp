...
rule pp = parse
	...
	if Hashtbl.mem ppmacros id then begin
	  ...
	  let ol = match o with "" -> [] | _ -> options (snd e) lexbuf in
  	  let ppf = Hashtbl.find ppenvs id in
          let contents =
            try
              Misc.IO.input_file fname
            with Sys_error _ ->
              error (snd e) (Printf.sprintf "`%s' cannot be loaded" fname)
          in
	    with_options ol (ppf !out) contents
	end else
	  print_string (lexeme lexbuf);
	pp lexbuf
      }
  ...
  | eof
      { () }
  | newline as c { newline lexbuf; print_string c; pp lexbuf }
  | _ as c
      {
	if c = '\n' then newline lexbuf;
