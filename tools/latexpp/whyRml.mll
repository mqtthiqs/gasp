  ...
  let why_rml_keywords =
    ...
    List.iter (fun s -> Hashtbl.add h s ())
      [
        ...
        "writes"; "prob"; "terminates"; "normal"
      ];
    h

  let is_keyword = Hashtbl.mem why_rml_keywords
...
{
  let why_rml_alltt fmt s =
    fprintf fmt "\\begin{alltt}";
    pp fmt (from_string s);
    fprintf fmt "\\end{alltt}%%\n"

  let why_rml_sf fmt s =
    fprintf fmt "\\bgroup\\sf\\medskip\\begin{flushleft}\n";
    start_of_line fmt (from_string s);
    fprintf fmt "\\end{flushleft}\\medskip\\egroup\\noindent\n"

  let () = Pp.add_pp_environment "why-rml-tt" why_rml_alltt
  let () = Pp.add_pp_environment "why-rml-sf" why_rml_sf
  let () = Pp.add_pp_environment "why-rml" why_rml_sf
  ...
  let () = Pp.add_pp_macro "why-rml-tt" texttt
  let () = Pp.add_pp_macro "why-rml-sf" textsf
  let () = Pp.add_pp_macro "why-rml" textsf
}
