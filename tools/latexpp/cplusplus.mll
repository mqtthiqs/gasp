...

{
  ...
  let ocaml_keywords =
    ...
    List.iter (fun s -> Hashtbl.add h s ())
      [
        "include"; "define"; "ifndef"; "endif"; "ifdef"; "end";
...
{
  let cplusplus_alltt fmt s =
    fprintf fmt "\\begin{alltt}";
    pp fmt (from_string s);
    fprintf fmt "\\end{alltt}%%\n"

  let cplusplus_sf fmt s =
    fprintf fmt "\\bgroup\\sf\\medskip\\begin{flushleft}\n";
    start_of_line fmt (from_string s);
    fprintf fmt "\\end{flushleft}\\medskip\\egroup\\noindent\n"

  let () = Pp.add_pp_environment "cplusplus-tt" cplusplus_alltt
  let () = Pp.add_pp_environment "cplusplus-sf" cplusplus_sf
  let () = Pp.add_pp_environment "cplusplus" cplusplus_sf
  ...
  let () = Pp.add_pp_macro "cplusplus-tt" texttt
  let () = Pp.add_pp_macro "cplusplus-sf" textsf
  let () = Pp.add_pp_macro "cplusplus" texttt
}
